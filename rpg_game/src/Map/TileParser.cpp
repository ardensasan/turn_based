				...
				if (map[x][y] != 0) {
					...
					for (tilesetIterator = tilesets.begin();tilesetIterator != tilesets.end();++tilesetIterator) {
						if (tilesetIterator->firstID <= map[x][y] && map[x][y] <= tilesetIterator->lastID) {
							...
							//TextureManager::GetInstance()->DrawTile(tilesetIterator->source, x, y, 32, 32, tileRow, tileCol);
							Tile _tile;
							...
							_tile.source = tilesetIterator->source;
...
void TileParser::Render() {
	...
	for (it = tiles.begin();it != tiles.end();++it) {
		TextureManager::GetInstance()->DrawTile(it->srcRect, it->dstRect, it->source);
	}
}
